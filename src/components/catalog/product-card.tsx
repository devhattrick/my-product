...
export function ProductCard({ product, categoryName, lineUrl }: ProductCardProps) {
  return (
    <Card className="group overflow-hidden border-white/80">
      <div className="relative m-4 overflow-hidden rounded-[1.75rem] border border-white/70 bg-[linear-gradient(180deg,#f9fafb_0%,#eef2ff_100%)]">
      ...
      <CardContent className="grid gap-5 pb-6">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-semibold text-slate-950">{product.name}</h3>
            </div>
        ...
        <div className="grid gap-3">
          {product.highlights.map((highlight) => (
            <div
              key={highlight}
              className="flex items-start gap-3 rounded-[1.25rem] border border-slate-100 bg-slate-50/80 px-4 py-3"
            >
